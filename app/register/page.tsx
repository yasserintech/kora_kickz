...
import { unavailableAvailability, getProgramGroupForProgramSlug } from "@/lib/programs"
import { getHydratedProgramBySlug, getHydratedProgramGroupBySlug, getProgramAvailability } from "@/lib/program-service"
...
export default async function RegisterPage({
  ...
  const availability = await getProgramAvailability(program.slug).catch(() => unavailableAvailability)
  ...
  return (
    <main className="min-h-screen bg-white py-16">
      <div className="container mx-auto space-y-8 px-4">
        ...
        <RegistrationFlow program={program} initialAvailability={availability} />
      </div>
    </main>
  )
}
